...
describe('Zoom level is calculated correctly in the store', () => {
    ...
    it('Doesn\'t allow negative zoom level, or non numerical value as a zoom level', async () => {
        ...
        expect(getZoom()).to.eq(validZoomLevel, 'Should not accept non numerical values as zoom level');
        await store.dispatch('setZoom', 'test');
        expect(getZoom()).to.eq(validZoomLevel, 'Should not accept non numerical values as zoom level');
        // checking with undefined or null
        await store.dispatch('setZoom', undefined);
        expect(getZoom()).to.eq(validZoomLevel, 'Should not accept undefined or null value as zoom level');
    ...
    it('Calculate resolution from zoom levels according to OGC standard (with 0.1% error margin)', async () => {
        ...
        expect(getResolution()).to.approximately(resolutionAtZoom10 * Math.cos(lat * Math.PI / 180.0), toleratedDelta)
        ...
        await store.dispatch('setLatitude', lat);
        expect(getResolution()).to.approximately(resolutionAtZoom2 * Math.cos(lat * Math.PI / 180.0), toleratedDelta)
    })
