...
export async function getProfileDataForChunk(
    ...
    if (!chunk?.coordinates) {
        ...
        throw new ElevationProfileError('Malformed chunk', new Error('network_error'))
    }
    return [
...
export default async (
    ...
    if (!profileCoordinates || profileCoordinates.length === 0) {
        ...
        throw new ElevationProfileError(errorMessage, new Error('could_not_generate_profile'))
    }
    ...
    for (const coordinates of coordinatesForSegments) {
        ...
        if (!coordinateChunks) {
            ...
            throw new ElevationProfileError(
                ...
                new Error('could_not_generate_profile')
            )
        }
    ...
    return {
        segments,
        metadata: getProfileMetadata(segments.flatMap((segment) => segment.points)),
    }
}
