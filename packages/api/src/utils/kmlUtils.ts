...
import type { DrawingIcon, DrawingIconSet } from '@/types/icons'
...
const EMPTY_KML_DATA: string = '<kml></kml>'
...
const LEGACY_ICON_XML_SCALE_FACTOR: number = 1.5
...
function parseKmlName(content: string): string | undefined {
 ...
 */
function getKmlExtent(content: string): FlatExtent | undefined {
 ...
 */
function getFeatureDescriptionMap(content: string): Map<string, string> {
    const features = kmlReader.readFeatures(content, {
        dataProjection: WGS84.epsg, // KML files should always be in WGS84
        featureProjection: WGS84.epsg,
    })
 ...
 */
function getFeatureType(kmlFeature: Feature): EditableFeatureTypes | undefined {
    ...
    if (!featureType || !['MARKER', 'ANNOTATION', 'LINEPOLYGON', 'MEASURE'].includes(featureType)) {
        log.info({
            title: 'kmlUtils / getFeatureType',
            titleColor: LogPreDefinedColor.Lime,
            messages: [
 ...
 */
function getTextScale(style: Style): number | undefined {
 ...
 */
function getIconStyle(style: Style): IconStyle | undefined {
 ...
 */
function parseIconUrl(url: string): IconArgs | undefined {
...
function getIconSize(iconStyle: IconStyle): FeatureStyleSize {
    ...
    if (iconScale) {
        return (
            featureStyleUtils.allStylingSizes.find((size) => size.iconScale === iconScale) ??
            featureStyleUtils.SMALL
        )
    }
    return featureStyleUtils.SMALL
}
...
function getFillColor(
    ...
    if (geometryType === 'Point' && iconArgs?.color) {
        return featureStyleUtils.getFeatureStyleColor([
            iconArgs.color.r,
            iconArgs.color.g,
            iconArgs.color.b,
        ])
 ...
 */
function getEditableFeatureFromKmlFeature(
    ...
    const style = featureStyleUtils.getStyle(kmlFeature, resolution)
    if (!style) {
        log.error({
            title: 'kmlUtils / getEditableFeatureFromKmlFeature',
            titleColor: LogPreDefinedColor.Lime,
    ...
    const textSize = featureStyleUtils.getTextSize(textScale)
    const textColor = featureStyleUtils.getTextColor(style)
    ...
    const iconSize = iconStyle ? getIconSize(iconStyle) : featureStyleUtils.MEDIUM
    ...
    const coordinates = featuresAPI.extractOlFeatureCoordinates(kmlFeature)
    ...
    if (iconArgs?.isLegacy && iconStyle && icon) {
        ...
        if (image instanceof IconStyle) {
            image.setSrc(iconsAPI.generateIconURL(icon, fillColor?.fill))
        }
    }
...
function detectTextPlacement(
    ...
    const [textPlacementX, textPlacementY] = featureStyleUtils.calculateTextXYOffset(
        textScale,
        iconScale,
        anchor,
        iconSize,
    ...
    const allTextPlacementsWithoutUnknown: TextPlacement[] = [
        'top-left',
        'top',
        'top-right',
        'left',
    ...
    for (const placementOption of allTextPlacementsWithoutUnknown) {
        const [xOffset, yOffset] = featureStyleUtils.calculateTextOffsetFromPlacement(
            textPlacementX,
            textPlacementY,
            placementOption
        )
...
function iconUrlProxyFy(
    ...
    if (!LOCAL_OR_INTERNAL_URL_REGEX.test(url)) {
        ...
        const proxyUrl = fileProxyAPI.proxifyUrl(url)
    ...
    if (kmlLayer.style === KMLStyle.GEOADMIN) {
        features.forEach((olFeature) => {
            ...
            if (editableFeature) {
                ...
                editableFeature.coordinates = featuresAPI.extractOlFeatureCoordinates(olFeature)
                olFeature.set('editableFeature', editableFeature)
            }
            olFeature.setStyle(featureStyleUtils.geoadminStyleFunction)
        })
    }
 ...
 */
function isKmlFeaturesValid(kmlData: string): boolean {
    try {
...
function isKml(fileContent: ArrayBuffer | string): boolean {
...
export const kmlUtils = {
    EMPTY_KML_DATA,
    LEGACY_ICON_XML_SCALE_FACTOR,
    parseKmlName,
    getKmlExtent,
    getFeatureDescriptionMap,
    getFeatureType,
    getTextScale,
    getIconStyle,
    parseIconUrl,
    getIcon,
    getFillColor,
    getEditableFeatureFromKmlFeature,
    iconUrlProxyFy,
    parseKml,
    isKmlFeaturesValid,
    unzipKmz,
