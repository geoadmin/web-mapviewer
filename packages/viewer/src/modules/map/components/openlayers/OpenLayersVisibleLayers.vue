...
const filteredVisibleLayers = computed(() => {
    ...
    if (
        drawingStore.overlay.show &&
        (drawingStore.onlineMode === OnlineMode.Online ||
            drawingStore.onlineMode === OnlineMode.OnlineWhileOffline) &&
    ...
    if (
        drawingStore.overlay.show &&
        !isOnlineMode(drawingStore.onlineMode, true) &&
        drawingStore.layer.temporaryKmlId
    ) {
        return layersStore.visibleLayers.filter(
