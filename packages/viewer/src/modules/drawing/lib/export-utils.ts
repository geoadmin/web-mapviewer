 ...
 * @param asTrack If true, exports as track (trk/trkpt), otherwise as route (rte/rtept)
 * @returns GPX string
 */
export function generateGpxString(
    projection: CoordinateSystem,
    features: Feature<Geometry>[] = [],
    asTrack: boolean = false
): string {
    const normalizedFeatures = features.map((feature) => {
        ...
        if (asTrack) {
            ...
            if (clone.getGeometry() instanceof LineString) {
                const coords = (clone.getGeometry() as LineString).getCoordinates()
            ...
            if (clone.getGeometry() instanceof MultiLineString) {
                const coords = (clone.getGeometry() as MultiLineString).getCoordinates()[0]
