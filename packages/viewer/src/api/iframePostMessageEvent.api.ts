...
/**
 * Events fired by the iFrame postMessage implementation:
 *
 * - 'gaChange': Event raised whenever the app changes its state (the URL changed). Payload of this
 *   event : a String with the new URL of the viewer
 * - 'gaFeatureSelection': Event raised when a feature has been selected. Will fire as many events as
 *   there are feature selected on the map (won't bundle all features into one event). Payload of
 *   this event : a JSON containing the layerId and featureId of the selected feature
 * - 'gaMapReady': Event raised when the map shown has finished loading and is now visible in the HTML
 *   DOM. No payload with this event.
 */
