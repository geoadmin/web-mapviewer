...
describe('WMTSCapabilitiesParser of wmts-ogc-sample.xml', () => {
    ...
    it('Get Layer Extent in LV95', () => {
        ...
        expect(externalLayers[0].extent).to.be.an('Array')
        expect(externalLayers[0].extent!.length).toBe(4)
        expected.forEach((value, index) => {
            expect(externalLayers[0].extent![index]).toBeCloseTo(value, 2)
        })
        ...
        expect(externalLayers[1].extent).to.be.an('Array')
        expect(externalLayers[1].extent?.length).toBe(4)
        expected.forEach((value, index) => {
        ...
        expect(externalLayers[2].extent).to.be.an('Array')
        expect(externalLayers[2].extent?.length).toBe(4)
        expected.forEach((value, index) => {
            expect(externalLayers[2].extent![index]).toBeCloseTo(value, 2)
        })
        ...
        expect(externalLayers[3].extent).to.be.an('Array')
        expect(externalLayers[3].extent?.length).toBe(4)
        expected.forEach((value, index) => {
            expect(externalLayers[3].extent![index]).toBeCloseTo(value, 2)
        })

        // Extent from matching BoundingBox
        expect(externalLayers[4].id).toBe('BlueMarbleFifthGenerationGE')
        expect(externalLayers[4].extent).toEqual([2484928.06, 1108705.32, 2514614.27, 1130449.26])
    })
    ...
    it('Get Layer Extent in WGS84', () => {
        ...
        // Extent from WGS84BoundingBox
        expect(externalLayers[0].id).toBe('BlueMarbleNextGenerationCH')
        expect(externalLayers[0].extent).toEqual([5.96, 45.82, 10.49, 47.81])

        // Extent from BoundingBox in WGS84
        expect(externalLayers[1].id).toBe('BlueMarbleSecondGenerationAG')
        expect(externalLayers[1].extent).toEqual([7.8, 47.09, 8.47, 47.64])

        // Extent from BoundingBox without CRS
        expect(externalLayers[2].id).toBe('BlueMarbleThirdGenerationZH')
        expect(externalLayers[2].extent).toEqual([8.3, 47.21, 9.05, 47.73])

        // Extent from the TileMatrixSet
        expect(externalLayers[3].id).toBe('BlueMarbleFourthGenerationJU')
        expect(externalLayers[3].extent).toEqual([6.81, 47.12, 7.56, 47.55])
    })
