...
describe('Test on legacy param import', () => {
    context('Coordinates import', () => {
        ...
        it('loads correctly if params are already behind the hash', () => {
            ...
            cy.getPinia().should((pinia) => {
                const positionStore = usePositionStore(pinia)
                expect(positionStore.zoom).to.eq(zoom)
                const centerEpsg43262 = positionStore.centerEpsg4326
        ...
        it('reproject LV95 coordinates param to EPSG:4326', () => {
            ...
            cy.getPinia().should((pinia) => {
                const positionStore = usePositionStore(pinia)
                expect(positionStore.zoom).to.eq(lv95zoom)
                ...
                const centerEpsg43263 = positionStore.centerEpsg4326
        ...
        it('center where expected when given a X, Y coordinate in LV95', () => {
            ...
            cy.getPinia().should((pinia) => {
                const positionStore = usePositionStore(pinia)
                const centerEpsg43264 = positionStore.centerEpsg4326
                // the app applies a rounding to the 6th decimal for lon/lat
        ...
        it('center where expected when given a X, Y coordinate in LV03', () => {
            ...
            cy.getPinia().should((pinia) => {
                const positionStore = usePositionStore(pinia)
                const centerEpsg43265 = positionStore.centerEpsg4326
                // the app applies a rounding to the 6th decimal for lon/lat
    ...
    context('Layers import', () => {
        ...
        it('is able to import an external KML from a legacy adminId query param with other layers', () => {
            ...
            cy.getPinia().should((pinia) => {
                const layersStore = useLayersStore(pinia)
                expect(layersStore.activeLayers).to.be.an('Array').length(3)
                const [wmsLayer, wmtsLayer, kmlLayer] = layersStore.activeLayers
                assertDefined(wmsLayer)
                expect(wmsLayer.id).to.eq('test.wms.layer')
                expect(wmsLayer.opacity).to.eq(0.6)
                expect(wmsLayer.isVisible).to.be.true
        ...
        it('External WMS layer', () => {
            cy.getExternalWmsMockConfig().then((mockConfig: ExternalWMSLayer[]) => {
                ...
                cy.getPinia().should((pinia) => {
                    const layersStore = useLayersStore(pinia)
                    expect(layersStore.activeLayers).to.be.an('Array').length(2)
                    const externalLayer = layersStore.activeLayers[1]
    ...
    context('3D import', () => {
        ...
        it('transfers camera parameter from legacy URL to the new URL', () => {
            ...
            cy.getPinia().should((pinia) => {
                ...
                const positionStore = usePositionStore(pinia)
                assertDefined(positionStore.camera)
    ...
    context('Time slider / Compare slider', () => {
        ...
        it('should show the time slider on startup when setting "time" in the URL', () => {
            cy.goToMapView({
                queryParams: {
            ...
            cy.get('[data-cy="time-slider-dropdown-input"]').should('have.value', 2019)
        })
    })
