 ...
 */
function enforceEndingSlashInUrl(url: string): string {
    if (url && !url.endsWith('/')) {
        return `${url}/`
    }
    return url
...
type UrlConfig = { [key in UrlBaseBackendService]: string }
 ...
 * endpoints
 */
const defaultBaseUrlConfig: UrlConfig = {
    /**
...
export function getDefaultBaseUrl(propertyName: UrlBaseBackendService): string {
    return defaultBaseUrlConfig[propertyName]
}

const baseUrlOverrides: Partial<UrlConfig> = {}

export function hasBaseUrlOverrides(): boolean {
    return Object.values(baseUrlOverrides).some((value) => !!value)
}

export function getBaseUrlOverride(propertyName: UrlBaseBackendService): string | undefined {
    return baseUrlOverrides[propertyName]
}

export function setBaseUrlOverrides(propertyName: UrlBaseBackendService, value: string) {
    baseUrlOverrides[propertyName] = enforceEndingSlashInUrl(value)
}

export function getBaseUrl(propertyName: UrlBaseBackendService): string {
...
export function getServiceAltiBaseUrl(): string {
    return getBaseUrl('serviceAlti')
}

export function getServiceSearchBaseUrl(): string {
