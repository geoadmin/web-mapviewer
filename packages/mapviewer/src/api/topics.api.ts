import type { GeoAdminGroupOfLayers, GeoAdminLayer, Layer } from '@geoadmin/layers'
...
const readTopicTreeRecursive = (
    ...
    if (node.category === 'topic') {
        const children: GeoAdminLayer[] = []
        node.children.forEach((topicChild) => {
            try {
                const child = readTopicTreeRecursive(topicChild, availableLayers)
                if (child.type === LayerType.GROUP) {
                    log.error(
                        'Node children are of mixed type, this should not happen',
                        node,
                        topicChild
                    )
                } else {
