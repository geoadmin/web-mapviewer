...
import type { ExternalLayer, ExternalWMSLayer, GeoAdminLayer, Layer } from '@geoadmin/layers'
...
import { allCoordinateSystems, CoordinateSystem, LV95 } from '@geoadmin/coordinates'
 ...
 * @param layer
 */
function getApi3TimeInstantParam(layer: GeoAdminLayer): string | null {
    if (
        layer.timeConfig?.currentTimeEntry &&
        ![ALL_YEARS_TIMESTAMP, CURRENT_YEAR_TIMESTAMP].includes(
            layer.timeConfig.currentTimeEntry.timestamp
        )
    ) {
...
async function identifyOnExternalWmsLayer(config: IdentifyConfig): Promise<LayerFeature[]> {
    ...
    if (coordinate.length === 2) {
        requestExtent = createPixelExtentAround({
...
function parseGeomAdminFeature(
    layer: GeoAdminLayer,
    featureMetadata: { [key: string]: any },
    ...
    if (featureMetadata.bbox) {
        featureExtent = flattenExtent(featureMetadata.bbox)
    }
