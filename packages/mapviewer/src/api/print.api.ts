...
import type { MFPReportResponse, MFPWmsLayer } from '@geoblocks/mapfishprint'
import type MFPBaseEncoder from '@geoblocks/mapfishprint/lib/types/MFPEncoder'
...
import {
    ...
    MFPEncoder,
    type MFPSpec,
    type MFPSymbolizerLine,
    type MFPSymbolizerPoint,
    type MFPSymbolizerText,
    requestReport,
...
const MAX_PRINT_SPEC_SIZE: number = 1 * 1024 * 1024
...
function validateBackendResponse(response: any): boolean {
    if (
        !('layouts' in response) ||
    ...
    const layouts = response.layouts.filter((layout: any) => {
        return (
            'name' in layout &&
    ...
    for (const attributes of layouts.map((layout: any) => layout.attributes)) {
    ...
    return new Promise((resolve, reject) => {
        axios
            ...
            .then((capabilities) => {
                ...
                resolve(
                    capabilities?.layouts.map((layout: any) => {
                        return new PrintLayout(
                            layout.name,
                            ...layout.attributes.map((attribute: any) => {
...
async function transformOlMapToPrintParams(olMap: Map, config: PrintConfig): Promise<MFPSpec> {
    ...
    try {
        ...
        const now = i18n.global.d(new Date(), 'datetime', i18n.global.locale.value)
        // const legend: MFPLegend
        const spec: MFPSpec = {
            attributes: {
                map: encodedMap,
 ...
 */
function printSpecReplacer(key: string, value: any): any {
