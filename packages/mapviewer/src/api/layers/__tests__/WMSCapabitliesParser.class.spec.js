...
describe('WMSCapabilitiesParser of wms-geoadmin-sample-sld-enabled.xml', () => {
    ...
    it('Parse layer legend', () => {
        ...
        expect(layer.legends.length).toBe(1)
        expect(layer.legends[0].url).toBe(
            'https://wms.geo.admin.ch/?SERVICE=WMS&REQUEST=GetLegendGraphic&ERSION=1.3.0&FORMAT=image/png&LAYER=ch.swisstopo-vd.official-survey&SLD_VERSION=1.1.0'
        )
        ...
        expect(layer.legends.length).toBe(1)
        expect(layer.legends[0].url).toBe(
            'https://wms.geo.admin.ch/?SERVICE=WMS&REQUEST=GetLegendGraphic&ERSION=1.3.0&FORMAT=image/png&LAYER=ch.swisstopo-vd.stand-oerebkataster&SLD_VERSION=1.1.0'
        )
    })
})
