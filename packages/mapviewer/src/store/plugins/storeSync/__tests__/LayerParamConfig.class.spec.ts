...
describe('External layer parsing with decodeUrlLayerId', () => {
    it('parses a KML layer correctly', () => {
        ...
        const result = decodeUrlLayerId(`${'KML'}|${encodeExternalLayerParam(kmlFileUrl)}`)
        expect(result.type).to.eq('KML')
    ...
    it('parses an external WMS layer correctly', () => {
        ...
        const result = decodeUrlLayerId(
            `${'WMS'}|${encodeExternalLayerParam(wmsBaseUrl)}|${encodeExternalLayerParam(wmsLayerId)}`
        )
        expect(result.type).to.equal('WMS')
    ...
    it('parses an external WMTS layer correctly', () => {
        ...
        const result = decodeUrlLayerId(
            `${'WMTS'}|${encodeExternalLayerParam(wmtsGetCapUrl)}|${encodeExternalLayerParam(wmtsLayerId)}`
        )
        expect(result.type).to.equal('WMTS')
