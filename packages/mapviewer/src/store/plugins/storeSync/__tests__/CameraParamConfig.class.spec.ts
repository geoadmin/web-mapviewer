...
import cameraParam, { readCameraFromUrlParam } from '@/store/plugins/storeSync/params/camera.param'
...
describe('CameraParamConfig class test', () => {
    ...
    describe('writing the query', () => {
        ...
        it('writes nothing if the value of a param is equal to zero', () => {
            ...
            ).to.eq(`${camera.x},,${camera.z},,${camera.heading},${camera.roll}`)
        })
    })
    ...
    describe('valuesAreDifferentBetweenQueryAndStore', () => {
        ...
        it('detects correctly when the values are the same', () => {
            expect(testInstance.valuesAreDifferentBetweenQueryAndStore({ camera: '1,1,1,1,1,1' }))
                .to.be.false
        })
    })
})
