...
function setCenterIfInBounds(store, center) {
    if (LV95.getBoundsAs(store.state.position.projection).isInBounds(center[0], center[1])) {
        ...
        store.dispatch('addErrors', {
            errors: [new ErrorMessage({ msg: 'geoloc_out_of_bounds' })],
            ...dispatcher,
        })
    }
}
...
const handlePositionError = (error, store, state, options = {}) => {
    ...
    switch (error.code) {
        case GeolocationPositionError.PERMISSION_DENIED:
            ...
            store.dispatch('addErrors', {
                errors: [new ErrorMessage({ msg: 'geoloc_permission_denied' })],
                ...dispatcher,
            })
            break
        case GeolocationPositionError.TIMEOUT:
            ...
            store.dispatch('addErrors', {
                errors: [new ErrorMessage({ msg: 'geoloc_time_out' })],
                ...dispatcher,
            })
            break
        default:
            ...
            if (errorCount < 3) {
                ...
                store.dispatch('addErrors', {
                    errors: [new ErrorMessage({ msg: 'geoloc_unknown' })],
                    ...dispatcher,
                })
                if (reactivate) {
