...
const dispatcher: ActionDispatcher = { name: 'click-on-map-management.plugin' }
    ...
    context.store.$onAction(({ after, name, args }) => {
        after(() => {
            if (
                ...
                const isCtrlLeftSingleClick =
                    clickInfo?.clickType === ClickType.CTRL_LEFT_SINGLECLICK
                const isContextMenuClick = clickInfo?.clickType === ClickType.CONTEXTMENU
                const isIdentifyingFeature =
                    clickInfo.clickType === ClickType.LEFT_SINGLECLICK ||
                    clickInfo.clickType === ClickType.CTRL_LEFT_SINGLECLICK ||
                    clickInfo.clickType === ClickType.DRAW_BOX
