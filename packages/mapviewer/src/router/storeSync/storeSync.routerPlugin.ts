...
import type { RouteLocationNormalized, RouteLocationNormalizedGeneric, Router } from 'vue-router'
...
import { STORE_DISPATCHER_ROUTER_PLUGIN } from '@/router/storeSync/UrlParamConfig.class'
import { MAP_VIEWS } from '@/router/viewNames.ts'
 ...
 */
function isRoutePushNeeded(store: Pinia, currentRoute: RouteLocationNormalizedGeneric): boolean {
...
function urlQueryWatcher(store, to, from) {
    ...
    storeSyncConfig.forEach((paramConfig) => {
        ...
        const setValueInStore = async (paramConfig, store, value) => {
            paramConfig.populateStoreWithQueryValue(to, store, value)
 ...
 */
const storeSyncRouterPlugin = (router: Router, store: Pinia) => {
    let unsubscribeStoreMutation
    router.beforeEach((to: RouteLocationNormalizedGeneric, from) => {
