        ...
        .forEach((kmlLayer) => {
            objects
                ...
                .forEach((kmlFeature) => {
                    ...
                    features.push(create3dKmlFeature(viewer, kmlFeature, kmlLayer))
                })
        })
...
function create3dKmlFeature(viewer, kmlFeature, kmlLayer) {
    ...
    if (kmlFeature.id.position) {
        ...
        featureCoordinates = proj4(WGS84.epsg, WEBMERCATOR.epsg, [
            // pos is in EPSG:4978 but proj4 does not support it so it is transformed to EPSG:4326 first
    ...
    const geogson = new GeoJSON().writeGeometryObject(geometry)
    const extent = bbox(geogson)
    return new LayerFeature({
        layer: kmlLayer,
        id: kmlFeature.id.id,
        data: {
