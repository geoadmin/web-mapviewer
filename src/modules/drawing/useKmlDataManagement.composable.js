...
export default function useSaveKmlOnChange(drawingLayerDirectReference) {
    ...
    async function saveDrawing(retryOnError = true) {
        try {
            ...
            const kml = generateKmlString(projection.value, drawingLayer.getSource().getFeatures())
            if (!activeKmlLayer.value) {
                // if we don't have an adminId then create a new KML File
                const kmlMetadata = await createKml(kml)
                const kmlLayer = new KMLLayer(
                    ...
                    kmlMetadata,
                    false, // external
                    false // isLoading, it is already loaded
                )
