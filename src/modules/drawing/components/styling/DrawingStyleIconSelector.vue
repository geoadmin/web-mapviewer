                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdown-size-selector">
                        <li>
                            <a
                                ...
                                :key="set.name"
                                class="dropdown-item"
                                :class="{ active: currentIconSetName === set.name }"
                                @click="currentIconSet = set"
                            >
                                {{ set.name }}
                            </a>
                        </li>
        ...
        >
            ...
            <div class="marker-icon-select-box" :class="{ 'one-line': !showAllSymbols }">
                <img
                    ...
                    @click="showAllSymbols && selectIcon(icon)"
                />
            </div>
...
export default {
    ...
    data: function () {
        return {
            ...
            currentIconColor: drawingStyleColors.find((color) => color.name === 'red'),
            currentIconSet: this.iconSets.find((set) => set.name === 'default'),
            currentIcon: this.iconSets.find((set) => set.name === 'default').icons[0],
            // default icon size is 1.0
            currentIconSize: drawingStyleSizes.find((size) => size.iconScale === 1.0),
            ...
            defaultIconSize: drawingStyleSizes.find((size) => size.iconScale === 1.0),
        }
    },
    computed: {
        currentIconSetName() {
