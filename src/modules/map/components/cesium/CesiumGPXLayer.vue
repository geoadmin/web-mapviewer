...
import useAddDataSourceLayer from '@/modules/map/components/cesium/utils/useAddDataSourceLayer.composable.js'
import log from '@/utils/logging.js'

const BILLBOARD_RADIUS = 8
...
function createRedCircleImage(opacity = 1) {
    ...
    canvas.width = BILLBOARD_RADIUS * 2
    canvas.height = BILLBOARD_RADIUS * 2

    // Draw a red circle on the canvas
    context.beginPath()
    context.arc(BILLBOARD_RADIUS, BILLBOARD_RADIUS, BILLBOARD_RADIUS, 0, 2 * Math.PI, false)
...
function createRedCircleBillboard(opacity = 1) {
    ...
    const billboardSize = BILLBOARD_RADIUS * 2
...
const { refreshDatSource } = useAddDataSourceLayer(
    viewer,
    createSource(),
...
watch(gpxData, () => refreshDatSource(createSource()))
</script>
<template>
    <slot />
</template>
