export class ClickInfo {
   ...
   *   click. Useful to determine if this was a quick touch or a long press on mobile (not the same
   *   feedback to the user, see click-on-map-management.plugin .js)
...
export default {
  state: {
    /**
     * @type Array<Feature> List of features to highlight on the map (features that are currently
     *   shown in the tooltip)
     */
    highlightedFeatures: [],
    /**
     * @type WMSLayer|WMTSLayer|GeoJsonLayer|AggregateLayer Layer that will be highlighted (put on
     *   top of all other layers) on the map. Used to show a layer from the search result when
     *   hovered by the mouse cursor.
     */
    highlightedLayer: null,
    /**
     * @type Boolean Whether the map is being dragged right now or not (if the mouse/touch is down
     *   and cursor moving)
     */
  ...
  actions: {
     ...
     * features are currently shown by the tooltip.
     *
     * @param commit
