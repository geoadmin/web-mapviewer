...
describe('Testing the report problem form', () => {
    ...
    it('test the report problem form UI, validations, and it backend interaction', () => {
        ...
        openForm()
        cy.get('[data-cy="report-problem-text-area-input"]').type(text)
        cy.get('[data-cy="report-problem-email-input"').should('be.empty')
        ...
        openForm()
        cy.get('[data-cy="report-problem-text-area-input"]').type(text)
        cy.get('[data-cy="report-problem-email-input"]').type('this.is.not.a.valid@email')
        cy.get('[data-cy="submit-button"]:visible').click()
        cy.get('[data-cy="report-problem-email-input"]').should('have.class', 'is-invalid')
        ...
        openForm()
        cy.get('[data-cy="report-problem-text-area-input"]').type(text)
        cy.get('[data-cy="report-problem-email-input"').type(validEmail)
        cy.get('[data-cy="submit-button"]').should('be.enabled')
        closeForm()

        cy.log('It is not possible to report a problem without filling the message')
        openForm()
        cy.get('[data-cy="report-problem-text-area-input"').should('be.empty')
        cy.get('[data-cy="report-problem-email-input"]').type(validEmail)
        cy.get('[data-cy="submit-button"]:visible').click()
        cy.get('[data-cy="report-problem-email-input"]').should('have.class', 'is-valid')
        cy.get('[data-cy="report-problem-text-area-input"]').should('have.class', 'is-invalid')
        cy.get('[data-cy="report-problem-failed-text"]').should('not.exist')
        closeForm()
        ...
        cy.get('[data-cy="report-problem-file-input"]').selectFile('@kmlFixture', {
            force: true,
        })
        cy.get('[data-cy="submit-button"]').click()
        cy.get('[data-cy="report-problem-file-input-text"]').should('have.class', 'is-valid')
    ...
    it('reports a problem with drawing attachment', () => {
        ...
        cy.get('[data-cy="report-problem-text-area-input"]')
            .should('be.visible')
            .should('have.value', text)
        cy.get('[data-cy="report-problem-email-input"]')
            .should('be.visible')
        ...
        cy.get('[data-cy="report-problem-form"]').should('be.visible')
        cy.get('[data-cy="drawing-header-title"]').should('not.exist')
        cy.get('[data-cy="report-problem-text-area-input"]').should('have.value', text)
        cy.get('[data-cy="report-problem-email-input"]').should('have.value', validEmail)
