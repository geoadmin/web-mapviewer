...
describe('Testing of the compare slider', () => {
    function expectCompareRatioToBeNull() {
        // when the compare Ratio is null, it should not exist instead
        cy.readStoreValue('state.ui').then((uiStore) => {
            cy.wrap(uiStore).its('compareRatio').should('not.exist')
        })
    }
    ...
    context('Tests to check if the compare slider is functional', () => {
        ...
        context('Moving the compare slider', () => {
            ...
            it('moves when we click on it and drag the mouse', () => {
                ...
                // initial slider position is width / 2 -20
                cy.get('[data-cy="compare_slider"]').then((slider) => {
                    cy.readStoreValue('state.ui.width').then((width) => {
                        cy.wrap(slider.position()['left']).should('eq', width / 2 - 20)
                    })
                })
                ...
                moveSlider(73)
                cy.get('[data-cy="compare_slider"]').then((slider) => {
                    cy.wrap(slider.position()['left']).should('be.gte', 72.8)
                    cy.wrap(slider.position()['left']).should('be.lte', 73.2)
                })
            })
            it('refuses to get out of bounds when we move around', () => {
                ...
                moveSlider(-10)
                cy.get('[data-cy="compare_slider"]').then((slider) => {
                    //an epsilon here : we noticed that it was at -5.99999
                    cy.wrap(slider.position()['left']).should('be.gte', -6.01)
                })
                ...
                cy.get('[data-cy="compare_slider"]').then((slider) => {
                    cy.readStoreValue('state.ui.width').then((width) => {
                        cy.wrap(slider.position()['left']).should('be.lte', width - 33.99)
                    })
                })
            })
        })
        ...
        */
        context.skip('Cutting layers', () => {
            it.skip('cuts from the correct spot until the end of the map, and only from the top layer', () => {
                ...
                cy.get('[data-cy="compare_slider"]').trigger('mousemove', {
                    screenX: cy.readStoreValue('state.ui.width').then((width) => {
                        return width
                    }),
                    screenY: 0,
                })
