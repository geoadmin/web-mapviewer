...
describe('Test the deploy script', () => {
  ...
  context('help', () => {
    ...
    it('Shows help when no target is specified', () => {
      cy.exec(`${scriptExec}`, { failOnNonZeroExit: false }).then((result) => {
    ...
    it('Shows help when --help param is used', () => {
      cy.exec(`${scriptExec} --help`).then((result) =>
        checkResult(result, { firstLine: helpFirstLine })
      )
    })
    it('Shows target help when --help param is used with a target', () => {
      cy.exec(`${scriptExec} dev --help`).then((result) =>
        checkResult(result, { contains: 'Deploy dist/* files to DEV bucket' })
      )
    })
  })
  context('targets', () => {
    it('Fails with exit code 1 when wrong command/target is entered', () => {
      cy.exec(`${scriptExec} a_totally_wrong_target`, { failOnNonZeroExit: false }).then((result) =>
    ...
    it('Fails with exit code 1 if more than 1 target is given', () => {
      cy.exec(`${scriptExec} dev int`, { failOnNonZeroExit: false }).then((result) =>
        checkResult(result, {
